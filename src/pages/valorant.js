...
function Valorant() {
    ...
    const modes = [
        ...
        <Classic key={'classic'} name={"valorant"} mode="classic" desc={"Find the daily agent"} data={data} fields={fields} first_tips={{name: "Role", key: "role"}} second_tips={{name: "Patch", key: "release_patch"}} />,
        <Findmode key={'name'} name={"valorant"} mode="name" desc={"Guess the agent from his real name"} data={filterByField(data, "real_name")} filter="real_name" />,
        <Findmode key={'picture'} name={"valorant"} mode="picture" desc={"Guess from a filtered image"} data={data} filter="image_url" />
    ];
